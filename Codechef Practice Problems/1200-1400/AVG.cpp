...
int main() {
   ...
   while(t--){
      lli n,k,v;
      cin>>n>>k>>v;
      lli missing_el_sum=(v*(n+k));
      lli a;
      for(lli i=0;i<n;i++){
         cin>>a;
         missing_el_sum-=a;
      }
      // cout<<abs((float)missing_el_sum/k - (missing_el_sum/k) )<<endl;
      if(missing_el_sum%k==0 and missing_el_sum/k>0)
         cout<<(int)(missing_el_sum/k)<<endl;
      else
         cout<<"-1"<<endl;
   }
