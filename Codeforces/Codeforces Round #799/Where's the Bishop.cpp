...
vlli getPosition(vector<vector<char>> &grid){
   vlli ans;
   for(lli i=0;i<8;i++){
      for(lli j=0;j<8;j++){
         if(i!=0 and i!=7 and j!=0 and j!=7 and grid[i][j]=='#' and grid[i+1][j+1]=='#' and grid[i-1][j-1]=='#' and grid[i+1][j-1]=='#' and grid[i-1][j+1]=='#'){
            ans.pb(i+1);
            ans.pb(j+1);
            return ans;
         }
