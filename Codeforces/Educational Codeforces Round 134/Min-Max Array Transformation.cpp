...
void solve(vlli &arr, vlli &brr){
   vector<int> ans1,ans2;
   int index=0, t = brr.size()-1;
   for(lli i=0;i<arr.size();i++){
      // cout<<"print = "<<arr[i]<<" "<<brr[index]<<endl;
      if(brr[index]>=arr[i])
         ans1.pb(brr[index]-arr[i]);
      else{
         while(brr[index]<arr[i])
            index++;
         ans1.pb(brr[index]-arr[i]);
      }
