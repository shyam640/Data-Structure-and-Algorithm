...
int main() {
   ...
   while(t--){
      ...
      for(lli i=0;i<n;i++){
         if(mp1[arr[i]]>0 and mp2[arr[i]]>0){
            mp1[arr[i]]--;
            mp2[arr[i]]--;
         }
      }
      ...
      for(lli i=0;i<n;i++){
         mp1[arr[i]]--;
         if(mp1[arr[i]]>0 and arr[i]>9){
            lli digits = log10(arr[i])+1;
            a1.pb(digits);
            d1[digits]++;
            ans++;
         }else if(arr[i]>0){
         ...
         mp2[brr[i]]--;
         if(mp2[brr[i]]>0 and brr[i]>9){
            lli digits = log10(brr[i])+1;
            a2.pb(digits);
            d2[digits]++;
            ans++;
         }else if(brr[i]>0){
      ...
      for(lli i=0;i<a1.size();i++){
         if(d1[a1[i]]>0 and d2[a1[i]]>0){
            d1[a1[i]]--;
            d2[a1[i]]--;
         }
      }
      ...
      // print_arr(a1,lli,0,a1.size()-1);
      // print_arr(a2,lli,0,a2.size()-1);

      for(lli i=0;i<a1.size();i++){
         if(d1[a1[i]]!=0 and a1[i]!=1){
            d1[a1[i]]--;
            ans++;
         }
      }
      
      for(lli i=0;i<a2.size();i++){
         if(d2[a2[i]]!=0 and a2[i]!=1){
            d2[a2[i]]--;
            ans++;
         }
      }
