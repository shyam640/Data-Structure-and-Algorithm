...
int main() {
   ...
   while(t--){
      ...
      bool rev = (s[0]>s[s.length()-1]);
      for(lli i=0;i<s.length();i++)
         mp[s[i]].pb(i+1);
      lli maxS = abs(s[s.length()-1]-s[0]);
      lli score = maxS;
      string t = s;
      sort(t.begin(),t.end());
      ...
      // string valid = "";
      // valid+=f[0];
      // for(lli i=1;i<t.length();i++){
      //    if(maxS<=0)
      //       break;
      //    valid+=t[i];
      //    maxS-=abs((int)(t[i]-t[i-1]));
      // }
      // cout<<valid<<endl;
      ...
      // cout<<valid<<endl;
      vlli ans;
      for(lli i=0;i<f.length();i++){
         vector<lli> idx = mp[f[i]];
         for(lli j=0;j<idx.size();j++){
            ans.pb(idx[j]);
         }
      }
